...
<template>
  <section id="investigations" class="section-shell investigations-section">
    <div class="section-heading">
      <p class="section-kicker">{{ t("investigations_heading") }}</p>
      <h2>{{ t("investigations_heading") }}</h2>
      <p>
        {{ t("investigations_subheading") }}. {{ t("learn_subheading2") }}
        <router-link to="/map-view" class="map-link">
          {{ t("learn_subheading3") }}
        </router-link>
      </p>
    </div>

    <div class="featured-grid investigations-grid">
      <router-link
      ...
      >
        ...
        <div class="featured-copy">
          <h3>{{ t(item.label) }}</h3>
          <span class="featured-link">{{ t("investigations_link") }}</span>
        </div>
      </router-link>
    </div>
    ...
    <div v-if="totalPages > 1" class="carousel-controls">
      <button type="button" class="carousel-button" @click="goPrevious">
        {{ t("investigations_previous") }}
      </button>

      <div class="carousel-dots" aria-label="Investigation pages">
        <button
        ...
        >
          <span class="sr-only">{{ t("investigations_page") }} {{ page }}</span>
        </button>
      </div>

      <button type="button" class="carousel-button" @click="goNext">
        {{ t("investigations_next") }}
      </button>
    </div>
  </section>
</template>
